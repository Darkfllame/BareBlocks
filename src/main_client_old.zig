...
const vertices = [_]Vertex{
    ...
    .{ .position = .{ 0.0, 0.5, 0 }, .color = .{ 0, 0, 1 } },
};
...
const App = struct {
    ...
    fn doxGpuInfo(self: *App) void {
        ...
        , .{
            ...
            props.device_id,                             utils.UUID.fromBytes(&props.pipeline_cache_uuid, .little),
    ...
    fn render(self: *App, cmd: vk.CommandBufferProxy) !void {
        ...
        if (!sdl.SDL_GetWindowSizeInPixels(self.window, &w, &h)) {
            return error.SDL;
        }
        const aspect = @as(f32, @floatFromInt(w)) / @as(f32, @floatFromInt(h));
        // const fov = 2 * std.math.atan(aspect / @tan(std.math.degreesToRadians(90) / 2.0));
        const proj = math.Mat4f.newPerspective(90, aspect, 0.01, 100);
        const view = math.Mat4f.newTranslation(.new(0, 0, -1));

        const now = Io.Timestamp.now(self.io, .boot);

        cmd.bindPipeline(.graphics, self.pipeline);
        cmd.pushConstants(self.pipeline_layout, .{ .vertex_bit = true }, 0, @sizeOf(PushConstantData), &PushConstantData{
            .proj = proj,
            .view = view,
            .model = .newRotation(.unitY, std.math.degreesToRadians(@as(f32, @floatFromInt(now.toMilliseconds())) / 90)),
        });
        cmd.bindVertexBuffers(0, (&self.vbuffer.handle)[0..1], &.{0});
        cmd.draw(vertices.len, 1, 0, 0);
    }
    ...
    pub fn create(
        ...
        errdefer self.swapchain.deinit(self.gpa);

        for (self.swapchain.tcmd_buffers[0..self.swapchain.getImageCount()], 0..) |cmdb, i| {
            logger.debug("tcmd_buffer[{d}] = {*}", .{ i, cmdb });
        }
        ...
        const mman = &self.device.mman;

        logger.debug("Creating vertex buffer", .{});

        self.vbuffer = try mman.allocBuffer(.vertex, @sizeOf(Vertex) * vertices.len, .last);
        errdefer mman.freeBuffer(self.vbuffer);
        {
            const tbuffer = try mman.allocBuffer(.transfer, self.vbuffer.size, .last);
            defer mman.freeBuffer(tbuffer);

            var vertices_reader = Io.Reader.fixed(@ptrCast(&vertices));
            try mman.copyToTransferBuffer(tbuffer, &vertices_reader);
            logger.debug("Transfered {Bi} of vertex data", .{self.vbuffer.size});

            const cmd = try self.swapchain.beginTransfer();
            cmd.copyBuffer(tbuffer.handle, self.vbuffer.handle, &.{vk.BufferCopy{
                .src_offset = 0,
                .dst_offset = 0,
                .size = self.vbuffer.size,
            }});
            try self.swapchain.endTransfer();
        }

        logger.debug("{f}", .{mman});

        if (!sdl.SDL_ShowWindow(self.window)) {
            logger.err("Couldn't show window: {s}", .{sdl.SDL_GetError()});
            return error.SDL;
        }
        self.start_time = .now(self.io, .boot);

        // if (true) return error.blablabla;
        return self;
    }
    ...
    pub fn destroy(self: *App) void {
        ...
        self.swapchain.deinit(self.gpa);
        self.device.deinit();
        instp.destroySurfaceKHR(self.swapchain.surface, null);
        if (is_debug) {
            self.vki.destroyDebugUtilsMessengerEXT(self.instance, self.debug_messenger, null);
        }
    ...
    pub fn tick(self: *App) !void {
        ...
        if (self.do_rendering) {
            const cmd = try self.swapchain.beginDraw(self.gpa);
            if (self.render(cmd)) |_| {
                try self.swapchain.endDraw(self.gpa);
            } else |e| {
                self.swapchain.cancelDraw();
        ...
        if (self.frame_delta >= std.time.ns_per_s) {
            ...
            logger.debug("[FPS: {d} ({f})] [VRAM: {Bi}/{Bi}] [VK RAM USAGE: {Bi}]", .{
                self.frames,           frame_time,
                vram_budget[0],        vram_budget[1],
                self.vkimpl_alloc_use,
            });
            self.frame_delta = 0;
            self.frames = 0;
        }
    ...
    pub fn onEvent(self: *App, ev: *sdl.SDL_Event) !enum { @"continue", success } {
        ...
        switch (ev.type) {
            ...
            sdl.SDL_EVENT_WINDOW_RESIZED, sdl.SDL_EVENT_WINDOW_DISPLAY_CHANGED => if (ev.window.windowID == windowID) {
                ...
                try self.swapchain.recreate(self.gpa, .{});
            },
